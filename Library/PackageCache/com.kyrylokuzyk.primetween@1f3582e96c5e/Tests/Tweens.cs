...
public partial class Tests {
    ...
    public void TweenTimeScaleWhileInSequence() {
        ...
        Sequence.Create(t);
        LogAssert.Expect(LogType.Error, "Setting timeScale is not allowed because this tween in a Sequence. Please use Sequence.timeScale instead.");
        t.timeScale = 2f;
    ...
    static void expectIsDeadError() => LogAssert.Expect(LogType.Error, Constants.isDeadMessage);
    ...
    static int tweensCapacity => PrimeTweenManager.Instance.tweens.Capacity;
    ...
    public IEnumerator CreateShakeWhenTweenListHasNull() {
        ...
        Tween.Delay(0.0001f)
            .OnComplete(() => {
                Assert.AreEqual(1, PrimeTweenManager.Instance.tweens.Count(_ => _ == null));
    ...
    public IEnumerator SetAllPaused() {
        ...
        if (tweensCount != 0) {
            var aliveCount = instanceTweens.Count(_ => _ != null && _._isAlive);
    ...
    public IEnumerator TweenCanBeNullInProcessAllMethod() {
        ...
        Tween.Custom(this, 0, 1, 0.0001f, delegate {
            ...
            Assert.AreEqual(0, PrimeTweenManager.Instance.tweens.Count(_ => _ == null));
    ...
    public IEnumerator TweenPropertiesDefault() {
        ...
        void validate(Tween t, bool isAlive) {
            if (!isAlive) {
                for (int i = 0; i < 8; i++) {
                    expectError();
                }
    ...
    public IEnumerator StopAtEvenOrOddCycle() {
        for (int i = 0; i < 10; i++) {
            var t = Tween.Rotation(transform, Vector3.one, Time.deltaTime * 0.9f, cycles: 20, cycleMode: CycleMode.Yoyo);
    ...
    public IEnumerator SetCycles() {
        var t = Tween.Rotation(transform, Vector3.one, Time.deltaTime, cycles: 10);
    ...
    [Test]
    public void OutBounceDuration() {
        for (int i = 0; i < 100; i++) {
            var duration = 0.1f + Random.value * 3;
            var stiffness = Random.value;
            Assert.AreEqual(duration, Tween.PositionOutBounce(transform, Vector3.one, duration, 1, stiffness: stiffness).duration, 0.0001);
            Assert.AreEqual(duration, Tween.LocalPositionOutBounce(transform, Vector3.one, duration, 1, stiffness: stiffness).duration, 0.0001);
        }
    }
    #endif
    ...
    public void ExperimentalDefineSet() {
        #if PRIME_TWEEN_EXPERIMENTAL
        Assert.Inconclusive("Please remove the PRIME_TWEEN_EXPERIMENTAL define and run all tests again.");
        #endif
    }
    ...
    public IEnumerator RecursiveKillAllCall() {
        yield return Tween.Custom(0, 1, 1f, val => {
            if (val != 0) {
                expectRecursiveKillError(() => Tween.StopAll());
            }
        }).ToYieldInstruction();
        
        yield return Tween.Custom(0, 1, 0.01f, _ => {
            expectRecursiveKillError(() => Tween.CompleteAll());
        }).ToYieldInstruction();
        LogAssert.NoUnexpectedReceived();
    }
    ...
    static void expectRecursiveKillError(Action action) {
        expectException<Exception>(action, "Please don't call Tween.StopAll/CompleteAll()");
    }
    ...
    [UnityTest]
    public IEnumerator DelayInterpolationFactorIsAlwaysZero() {
        var d = Tween.Delay(0.05f);
        while (d.isAlive) {
            Assert.AreEqual(0, d.interpolationFactor);
            yield return null;
        }
    }
    ...
    public IEnumerator StopCalledOnLastTweenFrameFromOnUpdate() {
        ...
        int numUpdated = 0;
        Tween tween = default;
        tween = Tween.Custom(0, 1, 3f / fps, delegate { })
    ...
    public IEnumerator OnUpdateDestroyedTarget() {
        ...
        yield return Tween.Delay(getDt() * 2)
    ...
    public IEnumerator OnUpdateException() {
        ...
        expectOnUpdateRemoved();
        int numCompleted = 0;
        yield return Tween.Position(transform, Vector3.one, 0.001f)
    ...
    [UnityTest]
    public IEnumerator ExtraCycles() {
        yield return null;
        {
            var zeroDurTween = Tween.ScaleY(transform, 2f, 0, cycles: int.MaxValue);
            yield return null;
            Assert.AreEqual(1, zeroDurTween.cyclesDone);
            zeroDurTween.Complete();
        }
        {
            float duration = getDt() / 100;
            var t = Tween.ScaleY(transform, 2f, duration, cycles: int.MaxValue);
            yield return null;
            Assert.AreEqual(Mathf.Floor(Time.deltaTime / duration), t.cyclesDone);
            t.Complete();    
        }
    }
    ...
    public IEnumerator ExecutionOrder() {
        ...
        void validate(int order) {
            ...
            Assert.AreEqual(order, i);
            i++;
        }
        {
            yield return Sequence.Create(Tween.Custom(0, 1, getDur(), delegate { }).OnComplete(() => validate(0)))
                .ChainCallback(() => validate(1))
                .Chain(Tween.Delay(getDur(), () => validate(2)))
                .Chain(Tween.Custom(0, 1, getDur(), delegate { }).OnComplete(() => validate(3)))
        ...
        }
        {
            ...
            for (int j = 0; j < iterations; j++) {
                var copy = j;
                if (Random.value > 0.5f) {
                    seq.Chain(Tween.Delay(getDur(), () => validate(copy)));
                } else {
                    seq.ChainCallback(() => validate(copy));
                }
            }
            seq.ChainCallback(() => {
                validate(iterations);
